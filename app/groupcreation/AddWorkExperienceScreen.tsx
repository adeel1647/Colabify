...
const AddWorkExperienceScreen = () => {
  ...
  const handleSaveEducation = async () => {
    // Trim jobTitle and company to remove spaces at the beginning and end
    ...
    try {
      const response = await fetch(`${API_URL}/api/users/${id}/workExperience`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Failed to add Work Experience:', data);
        Alert.alert('Error', data.message || 'Failed to add Work Experience.');
      }
    } catch (error) {
